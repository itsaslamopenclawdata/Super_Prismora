...
export default function VinylPrismoraPortfolio() {
  const router = useRouter();
  const [portfolio, setPortfolio] = useState<VinylItem[]>([]);
  ...
  const loadPortfolio = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const stored = localStorage.getItem('vinylPortfolio');
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await fetch(`/api/vinylprismora/portfolio/${id}`, {
        method: 'DELETE',
      });
  ...
  const sortedAndFilteredPortfolio = portfolio
    ...
    .sort((a, b) => {
      if (sortBy === 'dateAdded') {
        ...
        return a.details.artist.localeCompare(b.details.artist);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 dark:from-neutral-900 dark:via-purple-900/20 dark:to-pink-900/20 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <Button onClick={() => router.push('/vinylprismora/scan')} variant="primary" className="flex-1 min-w-[150px]">
            Add New Record
          </Button>
          <Button onClick={() => router.push('/vinylprismora/marketplace')} variant="secondary" className="flex-1 min-w-[150px]">
            Visit Marketplace
          </Button>
        </div>
        ...
        {sortedAndFilteredPortfolio.length === 0 ? (
          <EmptyState
            variant="default"
            size="lg"
            title="No Records in Portfolio"
            description="Start by scanning your first vinyl record to build your collection"
            action={
              <Button onClick={() => router.push('/vinylprismora/scan')} variant="primary">
                Scan First Record
              </Button>
            }
          />
            ...
            {sortedAndFilteredPortfolio.map((item) => (
              <Card key={item.id} className="bg-white/80 dark:bg-neutral-800/80 backdrop-blur-sm hover:shadow-lg transition-shadow">
                ...
                <CardContent className="p-4">
                  ...
                  <p className="text-sm text-purple-600 dark:text-purple-400 mb-2">{item.details.album}</p>
                  <div className="space-y-1 text-sm text-neutral-600 dark:text-neutral-400">
                    ...
                    <div className="flex justify-between">
                      <span>Label:</span>
                      <span className="font-medium">{item.details.label}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Est. Value:</span>
                  ...
                  <div className="mt-4 flex gap-2">
                    <Button
                      onClick={() => router.push(`/vinylprismora/item/${item.id}`)}
                      variant="secondary"
                      size="sm"
                      className="flex-1"
                    >
