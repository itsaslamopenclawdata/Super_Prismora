...
export default function NotePrismoraResult() {
  const router = useRouter();
  const [identification, setIdentification] = useState<NoteIdentification | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    const storedResult = sessionStorage.getItem('noteIdentificationResult');
    if (storedResult) {
  ...
  const handleRescan = () => {
    sessionStorage.removeItem('noteIdentificationResult');
    router.push('/noteprismora/scan');
  };

  const handleViewPortfolio = () => router.push('/noteprismora/portfolio');
  const handleMarketplace = () => router.push('/noteprismora/marketplace');
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 dark:from-neutral-900 dark:via-green-900/20 dark:to-teal-900/20 py-12 px-4">
      <div className="max-w-4xl mx-auto">
              ...
              <div className="space-y-4">
                <div>
                  <h2 className="text-2xl font-bold text-neutral-800 dark:text-neutral-200 mb-2">
                    {identification.details.series} {identification.details.type}
                  </h2>
                  <p className="text-xl text-green-600 dark:text-green-400 mb-2">
                    {identification.details.denomination}
                  </p>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between py-2 border-b border-neutral-200 dark:border-neutral-700">
                    <span className="text-neutral-600 dark:text-neutral-400">Denomination:</span>
                    <span className="font-semibold text-neutral-800 dark:text-neutral-200">{identification.details.denomination}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-neutral-200 dark:border-neutral-700">
                    <span className="text-neutral-600 dark:text-neutral-400">Series:</span>
                    <span className="font-semibold text-neutral-800 dark:text-neutral-200">{identification.details.series}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-neutral-200 dark:border-neutral-700">
                    <span className="text-neutral-600 dark:text-neutral-400">Type:</span>
                    <span className="font-semibold text-neutral-800 dark:text-neutral-200">{identification.details.type}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-neutral-200 dark:border-neutral-700">
                    <span className="text-neutral-600 dark:text-neutral-400">Seal Color:</span>
                    <span className="font-semibold text-neutral-800 dark:text-neutral-200">{identification.details.sealColor}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-neutral-200 dark:border-neutral-700">
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          <Button onClick={handleRescan} variant="secondary" className="flex-1 min-w-[150px]">Scan Another Note</Button>
        ...
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
          <p className="text-sm text-green-800 dark:text-green-300">
            <strong>Disclaimer:</strong> Identification and value estimates are AI-generated and may not be 100% accurate. For high-value notes, please consult a professional currency appraiser.
          </p>
        </div>
      </div>
    </div>
