...
export default function CardPrismoraPortfolio() {
  ...
  const loadPortfolio = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const stored = localStorage.getItem('cardPortfolio');
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await fetch(`/api/cardprismora/portfolio/${id}`, {
        method: 'DELETE',
      });
  ...
  const sortedAndFilteredPortfolio = portfolio
    ...
    .sort((a, b) => {
      if (sortBy === 'dateAdded') {
        ...
        return a.details.player.localeCompare(b.details.player);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-violet-50 dark:from-neutral-900 dark:via-blue-900/20 dark:to-violet-900/20 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-neutral-600 dark:text-neutral-400">Your Card Portfolio</p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <Button onClick={() => router.push('/cardprismora/scan')} variant="primary" className="flex-1 min-w-[150px]">Add New Card</Button>
          <Button onClick={() => router.push('/cardprismora/marketplace')} variant="secondary" className="flex-1 min-w-[150px]">Visit Marketplace</Button>
        </div>
        ...
        {sortedAndFilteredPortfolio.length === 0 ? (
          <EmptyState
            ...
            description="Start by scanning your first sports card to build your collection"
            action={<Button onClick={() => router.push('/cardprismora/scan')} variant="primary">Scan First Card</Button>}
          />
        ) : (
          <Grid cols={1} mdCols={2} lgCols={3} gap="md">
            {sortedAndFilteredPortfolio.map((item) => (
              <Card key={item.id} className="bg-white/80 dark:bg-neutral-800/80 backdrop-blur-sm hover:shadow-lg transition-shadow">
                ...
                <CardContent className="p-4">
                  <h3 className="text-lg font-semibold text-neutral-800 dark:text-neutral-200 mb-1">{item.details.player}</h3>
                  <p className="text-sm text-blue-600 dark:text-blue-400 mb-2">{item.details.year} {item.details.set}</p>
                  <div className="space-y-1 text-sm text-neutral-600 dark:text-neutral-400">
                    <div className="flex justify-between"><span>Team:</span><span className="font-medium">{item.details.team}</span></div>
                  ...
                  <div className="mt-4 flex gap-2">
                    <Button onClick={() => router.push(`/cardprismora/item/${item.id}`)} variant="secondary" size="sm" className="flex-1">View Details</Button>
